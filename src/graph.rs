...
impl Graph{
    ...
    pub fn shortest_path_a_star(&self, from: u32, to: u32) -> f32{
        if from==to {return 0.0;}
        if self.vertices.len() == 0 {return -1.0;}
        // Record nodes we dont need to revisit
        let mut visited_nodes = HashSet::new();
        // Record the currently queued nodes
        let mut queued_nodes = HashSet::new(); queued_nodes.insert(from);
        let end_pos = self.vertices[to as usize];
        // Min heap of (vertex, dist from start node, dist to end node (euclidean)) sorted by sum of second and third element
        let mut queue: Vec<(u32, f32, f32)> = vec![(from, 0.0, dist(&self.vertices[from as usize], &end_pos))];
        while !queue.is_empty(){
            let cur_node = pop_heap_v3(&mut queue);
            visited_nodes.insert(cur_node.0);
            // Go through each edge, adding it to the queue
            for (edge_vert, edge_len) in self.edges[cur_node.0 as usize].iter(){
                // Return if we have found the target node
                ...
                if !queued_nodes.contains(edge_vert) {
                    queue.push((*edge_vert, cur_dist, cur_end_dist));
                    let start_index = queue.len()-1;
                    heapify_up_v3(&mut queue, start_index);
                    queued_nodes.insert(*edge_vert);
                }
                // Search through the queue to see if we need to update the queued value
                let mut index = 0;
...
pub fn heapify_up_v3(vec: &mut Vec<(u32, f32, f32)>, mut index: usize){
    if index == 0 {return;}
    let mut parent;
    while index > 0 {
        parent = (index-1)/2;
        if vec[parent].2 + vec[parent].1 < vec[index].2 + vec[index].1 {return;}
        vec.swap(parent, index);
        index = parent;
    }
}
pub fn pop_heap_v3(vec: &mut Vec<(u32, f32, f32)>) -> (u32, f32, f32){
    if vec.len() == 1 {return vec.pop().unwrap();}
    let return_value = vec.swap_remove(0);
    let mut i = 0;
    loop{
        if vec.len() > i*2+2 {
            if vec[i].2 + vec[i].1 > vec[i*2+1].2 + vec[i*2+1].1 && vec[i*2+1].2+vec[i*2+1].1 < vec[i*2+2].2+vec[i*2+2].1{
                vec.swap(i, i*2+1);
                i = i*2+1;
                continue;
            }
            if vec[i].2+vec[i].1 > vec[i*2+2].2+vec[i*2+2].1 && vec[i*2+2].2+vec[i*2+2].1 < vec[i*2+1].2+vec[i*2+1].1{
                vec.swap(i, i*2+2);
                i = i*2+2;
                continue;
            }
        ...
        if vec.len() > i*2+1 {
            if vec[i].2+vec[i].1 > vec[i*2+1].2+vec[i*2+1].1 {
                vec.swap(i, i*2+1);
                i = i*2+1;
                continue;
            }
